...
class DayBook extends CI_Controller{
	...
	function rSaleList(){
		...
		if(strlen($this->input->post("sdt")) > 0){
			...
			$list = $this->db->query("SELECT SUM(product_quantity) as quant,`product_id` FROM product_sale WHERE ref='$id' AND date >= '$sdt' AND date <= '$edt' GROUP BY product_id");
			$data['list'] = $list;
			$this->db->where("id",$id);
			$val = $this->db->get("ref")->row()->dis;
			$data['val'] = $val;
			$data['isPost'] = true;
		}else{
			$data['isPost'] = false;
